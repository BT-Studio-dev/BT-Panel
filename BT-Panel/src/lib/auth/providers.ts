 ...
 *
 * Each app federates to the shared **auth broker** (`GROK_AUTH_ISSUER`),
 * which holds the real Google/X secrets. The app never sees them — it only
 * knows its own per-app client id/secret and which upstream to ask the broker
 * for (`idp`).
 *
...
/**
 * Upstream OAuth providers federating through the broker.
 *
 * Google and X (Twitter) federation has been removed for the z.ai build —
 * sign-in is local email/password only (see `email-password.ts` and the
 * seeded `admin` account). Add entries back here to re-enable brokered
