...
// Seed the default `admin` / `admin` user on first start (z.ai build). The
// seed self-waits on `ensureDbReady` and is a no-op once any user exists, so
// it's safe to fire eagerly. Failures are logged but not awaited here — the
// panel will surface a "sign-in failed" error if the seed truly broke, and
// the next page load retries (the seed promise clears itself on rejection).
void seedAdminUser().catch(() => {
  /* error already logged inside seedAdminUser */
});
...
// `*.e2b.app` covers this workspace's Arena sandbox previews, whose hosts are
// `<port>-<sandboxId>.e2b.app` (the broker matching list itself stays tied to
// the shared preview client's `*.grok-sandbox.com`).
const previewAllowedHosts: string[] = [...PREVIEW_ALLOWED_HOSTS, "*.e2b.app"];
// Loopback dev/preview origins on ANY port: the live-preview contract binds
// :8080, but a sandbox preview relay (or plain `vite dev --port`) may bind
// another port. `*` in the port position is better-auth's glob wildcard and
// matches the full origin — loopback-only, so the trust surface stays local.
...
/**
 * Trusted origins for credentialed auth POSTs (CSRF / origin check).
 *
 * better-auth merges these per request with the origins it already derives
 * from the dynamic baseURL config (allowedHosts patterns + fallback). The
 * function adds a same-host allowance: the preview relay forwards the
 * browser's own Host / X-Forwarded-Proto, so a request whose Origin equals
 * its forwarded host is first-party by construction. Loopback stays trusted
 * on any port for local dev (see LOCAL_DEV_ORIGINS).
 */
const trustedOrigins = async (
  ...
  if (forwardedHost) {
    if (forwardedProto) {
      ...
      // Some preview relays (e2b.app) preserve the forwarded Host but DROP
      // X-Forwarded-Proto. Trust both schemes of OUR OWN host: a cross-site
      // attacker's Origin carries a different host, so it still can't match
      // — only first-party requests pass.
      origins.push(`https://${forwardedHost}`, `http://${forwardedHost}`);
    }
  }
  return origins;
};
...
// Only a real postgres:// URL can drive `pg`. Anything else (e.g. a `file:`
// SQLite URL inherited from a sibling scaffold's .env) must not reach the
// Pool — fall back to the embedded PGlite the live-preview contract uses.
const databaseIsPostgres =
  databaseUrl !== undefined && /^postgres(ql)?:\/\//i.test(databaseUrl);
if (databaseUrl && !databaseIsPostgres) {
  console.warn(
    `[auth] DATABASE_URL scheme "${databaseUrl.split(":")[0]}:" is not postgres:// — using embedded PGlite instead`,
  );
}
const database = databaseIsPostgres
...
export const auth = betterAuth({
  ...
  // Local email/password — toggled via `./email-password`. The z.ai build lowers
  // the minimum password length from the Better Auth default (8) to 4 so the
  // seeded `admin` account (`admin` / `admin`) can sign in. The 8-character
  // floor was only ever a sign-up form guard, not a security invariant of the
  // scaffold; raise it back to 8 (or higher) if you re-enable public sign-up.
  ...(emailAndPasswordEnabled
    ? { emailAndPassword: { enabled: true, minPasswordLength: 4, maxPasswordLength: 128 } }
    : {}),
